...
def make_matrix_from_diag(d):
    return ti.Matrix([[d[0], 0.0], [0.0, d[1]]])
...
def substep():
    ...
    for p in x:  # Particle state update and scatter to grid (P2G)
        ...
        stress = 2 * mu * (F[p] - U @ V.transpose()) @ F[p].transpose() + ti.Matrix.identity(float, 2) * lam * J * (J - 1)
...
gui = ti.GUI("Taichi MPM with Von Mises", res=512, background_color=0x112F41)
reset()
gravity[None] = [0, -1]
...
for frame in range(20000):
    ...
    for s in range(int(2e-3 // dt)):
        substep()
    gui.circles(x.to_numpy(), radius=1.5, color=color.to_numpy())
    gui.show(
